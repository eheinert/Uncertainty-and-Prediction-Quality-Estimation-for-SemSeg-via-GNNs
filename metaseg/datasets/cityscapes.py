...
class Cityscapes(collections.abc.Mapping):
    """
    ...
    def __init__(
        ...
        mode: str = "meta_train",
        splits: dict = {"meta_train":"val",
                       "meta_infer":"test"}
    ):
        self.mode = mode
        self.split = splits[mode]
        ...
        for city in self.images_root.glob("*"):
            target_dir = self.target_root / city.name
            probs_dir = self.probs_root / city.name
            base_dir = Path(self.split) / city.name
            for file_path in city.glob("*.png"):
                self.all_images.append(file_path)
                self.targets.append(target_dir / f"{str(file_path.stem)}_gtFine_labelTrainIds.png".replace("_leftImg8bit", ""))
                self.probabilities.append(probs_dir / f"{file_path.stem}.npy".replace("_leftImg8bit", ""))
                self.all_basenames.append(base_dir / f"{file_path.stem}".replace("_leftImg8bit", ""))

    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        probs = np.load(self.probabilities[index])
        target = None
        if self.mode in ["meta_train","meta_val"]:
